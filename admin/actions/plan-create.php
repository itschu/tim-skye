...
try {
    // Insert new plan (id is auto-generated)
    $insert_id = db_insert(
        'investment_plans',
        [
            'name' => $name,
    ...
    $_SESSION['success'] = __('Investment plan created successfully');
    header('Location: /admin/plans');
    exit;
} catch (Exception $e) {
    error_log('Plan create error: ' . $e->getMessage(), 3, '../logs/db-errors.log');
    $_SESSION['error'] = __('An error occurred while creating the plan');
    header('Location: /admin/plans');
    exit;
}
