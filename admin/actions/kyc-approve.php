...
try {
    ...
    // Update KYC row to approved (single-row schema)
    db_update(
        'kyc_documents',
        [
            'status' => 'approved',
    ...
    $success_msg = __('KYC submission approved and user marked verified');

    // Send approval email (if enabled)
    $email_kyc_approved_user = get_setting('email_kyc_approved_user', 'yes');
    if ($email_kyc_approved_user === 'yes') {
        ...
        try {
            ...
            error_log("Failed to send KYC approval email: " . $e->getMessage(), 3, __DIR__ . '/../../logs/email-errors.log');
        }
    }
    ...
    error_log('KYC approve error: ' . $e->getMessage(), 3, '../logs/db-errors.log');
    $_SESSION['error'] = __('An error occurred while approving the document');
    header('Location: /admin/kyc-review');
    exit;
}
