...
/**
 * Referral withdrawal submit handler
 * Processes referral withdrawal requests with fee calculation and balance validation.
 */
...
Please review and process this withdrawal in the admin panel."),
    ...
    $_SESSION['success'] = __('Referral withdrawal request submitted successfully. Net amount after fee: ') .
        format_money($net_amount);
    header('Location: /user/referrals?success=1');
    exit;
} catch (Exception $e) {
    $db->rollBack();
    error_log('[Withdrawal Error] ' . $e->getMessage(), 3, __DIR__ . '/../logs/db-errors.log');
    $_SESSION['error'] = __('Failed to process referral withdrawal request. Please try again later.');
    header('Location: /user/referrals');
    exit;
}
