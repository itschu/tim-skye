...
CREATE TABLE `investments` (
  ...
  `total_profit_earned` decimal(65,30) NOT NULL DEFAULT 0.00,
...
CREATE TABLE `transactions` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `user_id` int(11) NOT NULL,
  `type` enum('deposit','withdrawal','profit','referral','investment','refund','cancellation_penalty','referral_fund') NOT NULL,
  `amount` decimal(65,30) NOT NULL,
...
CREATE TABLE `deposits` (
  ...
  `user_id` int(11) NOT NULL,
  `amount` decimal(65,30) NOT NULL,
  `fee_amount` decimal(65,30) NOT NULL DEFAULT 0.00,
  `net_amount` decimal(65,30) NOT NULL DEFAULT 0.00,
  ...
  `created_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `local_currency_amount` DECIMAL(65,30) DEFAULT NULL,
  `local_currency_code` VARCHAR(3) DEFAULT NULL,
  `exchange_rate_used` DECIMAL(65,30) DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `user_id` (`user_id`),
  KEY `status` (`status`),
...
CREATE TABLE `withdrawals` (
  ...
  `status` enum('pending','approved','rejected') NOT NULL DEFAULT 'pending',
  `source` enum('balance','referral') NOT NULL DEFAULT 'balance',
  `rejection_reason` text DEFAULT NULL,
  `admin_id` int(11) DEFAULT NULL,
  `created_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
